...
- Keyboard input in the cloud Desktop (noVNC) pane: the sim listens on `document`, and XFCE here uses click-to-focus, so keystrokes only reach the sim while the **Chrome window holds X input focus**. If the keyboard seems dead, click once inside the Chrome 3D canvas to focus it (clicking the XFCE desktop/panel or losing window focus silently drops all key input — verified: pressing `N` toggles day/night only when Chrome is focused). This is a VNC focus behavior, not an app bug; keyboard delivery itself works. `computerUse`/`xdotool` inject keys straight to the focused window, so automated tests aren't affected. Note the app clears held keys on `window` blur, so it also stops on focus loss.
