...
export function geodeticToEcef(latDeg: number, lonDeg: number, height: number, out: Ecef = { x: 0, y: 0, z: 0 }): Ecef {
...
/** ECEF metres → geodetic (degrees, metres). Bowring-ish closed form. */
export function ecefToGeodetic(x: number, y: number, z: number, out: Geodetic = { lat: 0, lon: 0, height: 0 }): Geodetic {
...
export function makeEnuBasis(lat0: number, lon0: number, height0: number = 0): EnuBasis {
    ...
    // ECEF→ENU: rows are e, n, u
    const ecefToEnu = new Float64Array([
        ex, ey, ez,
        nx, ny, nz,
        ux, uy, uz,
    ]);
    // ENU→ECEF: columns are e, n, u (= transpose)
    const enuToEcef = new Float64Array([
        ex, nx, ux,
        ey, ny, uy,
        ez, nz, uz,
    ]);
    return { origin, lat0, lon0, ecefToEnu, enuToEcef };
}
...
/**
 * Approximate geodetic from local ENU on the tangent plane (height ≈ 0),
 * then refine height separately via a HeightSource.
 */
